...
from config import (
    ...
    REQUEST_DELAY,
    REQUEST_TIMEOUT,
    NAVIGATION_DELAY_MIN,
    NAVIGATION_DELAY_MAX,
)
import random
...
class OttawaRecBookingScraper:
    ...
    def _extract_page_id(self, html: str, url: str = None) -> Optional[str]:
        ...
        for script in scripts:
            if script.string:
                ...
                matches = re.findall(
                    r'[Pp]ageId["\']?\s*[:=]\s*["\']([a-f0-9-]+)["\']', script.string
                )
                if matches:
                    return matches[0]
                # Also try PageId
    ...
    def _parse_booking_page_structure(self, html: str) -> Dict:
        ...
        """
        soup = BeautifulSoup(html, "html.parser")
        structure = {"buttons": [], "links": [], "forms": [], "page_type": "unknown"}

        # Determine page type
        if "TimeSelection" in html or soup.find("div", class_="date"):
            structure["page_type"] = "time_selection"
        elif "SlotCountSelection" in html or soup.find(
            "input", {"name": "reservationCount"}
        ):
            structure["page_type"] = "slot_count_selection"
        elif "StartReservation" in html or soup.find_all(
            "button", class_=re.compile(r"activity|sport", re.I)
        ):
        ...
        buttons = soup.find_all(
            ["button", "a", "input"], attrs={"type": ["button", "submit"]}
        )
        buttons.extend(
            soup.find_all(["a", "button"], class_=re.compile(r"btn|button|link", re.I))
        )
        ...
        for btn in buttons:
            ...
            button_id = (
                btn.get("data-buttonid")
                or btn.get("data-button-id")
                or btn.get("data-id")
                or btn.get("id")
            )
        ...
        links = soup.find_all(
            "a",
            href=re.compile(
                r"ReserveTime|StartReservation|SlotCount|TimeSelection", re.I
            ),
        )
        for link in links:
            link_info = {
        ...
        try:
            ...
            if extracted_page_id:
                ...
                if old_page_id != extracted_page_id:
                    logger.info(
                        f"Extracted NEW page ID: {self.current_page_id} (was {old_page_id})"
                    )
                else:
                    logger.info(
                        f"Extracted page ID matches config: {self.current_page_id}"
                    )
            else:
                # Fall back to config value if extraction fails
                logger.warning(
                    f"Could not extract page ID from page HTML/URL, using config value: {self.current_page_id}"
                )
            ...
            self._save_navigation_step(
                "step1_initial_page", response.text, response.url
            )
            ...
            if extracted_buttons:
                ...
                logger.info(
                    f"Extracted {len(extracted_buttons)} activity buttons from page: {list(extracted_buttons.keys())}"
                )
            ...
            logger.info(
                f"Session initialized - Session ID: {self.current_session_id[:20] if self.current_session_id else 'None'}..., Page ID: {self.current_page_id}"
            )
            logger.info(f"Initial page URL: {self.initial_page_url}")
            logger.info(
                f"Button IDs set from page: {list(self.activity_button_ids.keys())}"
            )
            ...
            logger.error(
                f"HTTP error initializing session: {e.response.status_code}\n{error_details}"
            )
            return False
    ...
    def _extract_activity_buttons(self, html: str) -> Dict[str, str]:
        ...
        # Use center-specific match patterns from config.
            ...
            for activity_type, patterns in activity_patterns.items():
                for pattern in patterns:
                    if re.search(pattern, text, re.I):
                        ...
                        button_id = (
                            candidate.get("data-buttonid")
                            or candidate.get("data-button-id")
                            or candidate.get("data-id")
                        )
                        ...
                        if button_id and re.match(r"^[a-f0-9-]{36}$", button_id):
                            if activity_type not in activity_buttons:
                                ...
                                logger.info(
                                    f"Extracted button ID for {activity_type}: {button_id} from text: '{candidate.get_text(strip=True)[:50]}'"
                                )
                            break

        return activity_buttons

    def _extract_button_id_from_page(
        self, html: str, activity_name: str
    ) -> Optional[str]:
        ...
        activity_keywords = {
            ...
            "badminton-doubles-family": ["badminton", "family"],
            "pickleball": ["pickleball"],
        }

        keywords = activity_keywords.get(
        ...
        for keyword in keywords:
            ...
            for element in elements:
                ...
                if parent:
                    ...
                    button_id = (
                        parent.get("data-buttonid")
                        or parent.get("data-button-id")
                        or parent.get("data-id")
                        or parent.get("onclick")
                    )
    ...
    def select_activity(self, activity_type: str = "badminton-16+") -> bool:
        ...
        try:
            ...
            logger.info(
                f"Navigating to select activity {activity_type} with buttonId={button_id}"
            )
            ...
            if response.status_code in [401, 403]:
                logger.error(f"Authentication error: HTTP {response.status_code}")
                self._save_navigation_step(
                    f"step2_{activity_type}_auth_error", response.text, response.url
                )
                return False
            ...
            self._save_navigation_step(
                f"step2_{activity_type}_after_select", response.text, response.url
            )
            self.last_page_html = response.text
            ...
            error_details = traceback.format_exc()
            logger.error(
                f"HTTP error selecting activity {activity_type}: {e.response.status_code}\n{error_details}"
            )
            ...
            if e.response:
                try:
                    self._save_navigation_step(
                        f"step2_{activity_type}_error_{e.response.status_code}",
                        e.response.text,
                        e.response.url,
                    )
                    screenshot_path = self.save_timeslots_html(
                        step_name=f"{activity_type}_select_error_{e.response.status_code}",
            ...
            error_details = traceback.format_exc()
            logger.error(
                f"Failed to select activity {activity_type}: {str(e)}\n{error_details}"
            )
            return False

    def set_group_size(
        self, activity_type: str, group_size: int = DEFAULT_GROUP_SIZE
    ) -> bool:
        ...
        try:
            ...
            if (
                ...
                response = self.session.get(
                    self.last_activity_url, timeout=REQUEST_TIMEOUT
                )
                response.raise_for_status()
                self._save_navigation_step(
                    f"step2b_{activity_type}_slot_count_page",
                    response.text,
                    response.url,
                )
                self.last_page_html = response.text
                ...
                logger.info(
                    "Already on TimeSelection page, group size may be set during booking"
                )
                return True
            else:
                ...
                if response.status_code == 404:
                    logger.info(
                        f"SlotCountSelection returned 404, skipping group size step for {activity_type}"
                    )
                    return True
            ...
            response = self.session.post(
                post_url, data=form_data, timeout=REQUEST_TIMEOUT, allow_redirects=True
            )

            # If 404 on POST, the endpoint doesn't exist for this activity
            if response.status_code == 404:
                logger.info(
                    "Group size POST returned 404, endpoint may not exist for this activity"
                )
            ...
            # Save this navigation step
            self._save_navigation_step(
                f"step2c_{activity_type}_after_group_size", response.text, response.url
            )
            self.last_page_html = response.text
        ...
        try:
            ...
            if navigate:
                ...
                if not self.select_activity(activity_type):
                    ...
                    if (
                        hasattr(self, "screenshots")
                        and activity_type in self.screenshots
                    ):
                ...
                if (
                    ...
                    not self.last_activity_url
                    or "TimeSelection" not in self.last_activity_url
                ):
            ...
            if response.status_code in [401, 403]:
                self._save_navigation_step(
                    f"step3_{activity_type}_auth_error", response.text, response.url
                )
                return {
                    "success": False,
                    "slots": [],
            ...
            # Save this navigation step
            self._save_navigation_step(
                f"step3_{activity_type}_time_selection", response.text, response.url
            )
            self.last_page_html = response.text

            # Validate that we're on the TimeSelection page (simplified check)
            if (
                "TimeSelection" not in response.url
                and "TimeSelection" not in response.text[:1000]
            ):
                self._save_navigation_step(
                    f"step3_{activity_type}_unexpected_page",
                    response.text,
                    response.url,
                )
                return {
            ...
            screenshot_path = self.save_timeslots_html(
                step_name=f"step4_{activity_type}_final_slots",
            ...
            return {
                ...
                "slots": slots,
                "message": (
                    f"Found {len(slots)} available slot(s)"
                    if slots
                    else "No available slots found"
                ),
                "screenshot": screenshot_path,
            }
        except requests.exceptions.HTTPError as e:
            ...
            error_details = traceback.format_exc()
            logger.error(
                f"HTTP error getting available slots for {activity_type}: {status_code} - {str(e)}\n{error_details}"
            )
            ...
            if e.response:
                try:
                    ...
                    self._save_navigation_step(
                        f"step3_{activity_type}_error_{status_code}",
                        e.response.text,
                        e.response.url,
                    )
                    screenshot_path = self.save_timeslots_html(
                        step_name=f"{activity_type}_error_{status_code}",
                        html_content=e.response.text,
                    )
                    if screenshot_path:
                        if not hasattr(self, "screenshots"):
            ...
            error_details = traceback.format_exc()
            logger.error(
                f"Network error getting available slots for {activity_type}: {str(e)}\n{error_details}"
            )
            return {
                "success": False,
                "slots": [],
            ...
            error_details = traceback.format_exc()
            logger.error(
                f"Failed to get available slots for {activity_type}: {str(e)}\n{error_details}"
            )
            return {
                "success": False,
                "slots": [],
    ...
    def _save_navigation_step(
        self, step_name: str, html: str, url: str = None
    ) -> Optional[str]:
    ...
    def save_timeslots_html(
        self, activity_type: str = None, html_content: str = None, step_name: str = None
    ) -> Optional[str]:
            ...
            for time_link in time_links:
                ...
                if match:
                    ...
                    if not queue_id or not date_time or not time_hash:
                        logger.warning(
                            f"Invalid slot data extracted: queue_id={queue_id}, date_time={date_time}, time_hash={time_hash}"
                        )
                        continue

                    time_text = time_link.find("span", class_="available-time")
                    time_display = (
                        time_text.get_text(strip=True) if time_text else date_time
                    )
    ...
    def find_next_available_slot(
        self, activity_type: str = "badminton-16+", group_size: int = DEFAULT_GROUP_SIZE
    ) -> Optional[Dict]:
    ...
    def get_contact_info_fields(
        self, activity_type: str, slot_data: Dict, group_size: int = DEFAULT_GROUP_SIZE
    ) -> Dict:
    ...
    def submit_contact_info(self, activity_type: str, field_values: Dict) -> Dict:
        ...
        try:
            ...
            if "confirm" in response.url.lower() or "success" in response.url.lower():
                ...
                error_messages = soup.find_all(
                    ["div", "span"], class_=re.compile(r"error|validation", re.I)
                )
                error_text = " ".join(
                    [
                ...
                return {
                    "success": False,
                    "message": error_text
                    or "Form submission failed. Please check your information.",
                    "error_type": "validation_error",
                    "url": response.url,
                }
            else:
